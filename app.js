...
function login(username) {
    ...
    // Ensure user data structure exists (but don't reload sample data)
    if (!db.data[currentUser]) {
        ensureUserDataStructure(db);
        saveDB();
        console.log('User data structure created for:', currentUser);
    } else {
        console.log('User data structure already exists for:', currentUser);
    }
...
function showModule(moduleName) {
    ...
    // Set active nav button (fix for event.target issue)
...
function clearUserData() {
    if (confirm(`Are you sure you want to clear all data for ${currentUser}? This cannot be undone!`)) {
        ...
        if (activeBtn) {
            const moduleText = activeBtn.textContent.trim().toLowerCase();
            if (moduleText.includes('poster')) loadPosters();
            else if (moduleText.includes('ward')) loadWardMaps();
            else if (moduleText.includes('skywalk')) loadSkywalks();
            else if (moduleText.includes('survey')) loadSurveys();
            else if (moduleText.includes('interview')) loadInterviews();
            else if (moduleText.includes('mapillary')) loadMapillary();
            else if (moduleText.includes('problem')) loadProblems();
        }
...
// Add function to completely reset localStorage (for testing)
function resetAllData() {
    if (confirm('This will delete ALL data for ALL users. Are you sure?')) {
        localStorage.removeItem('urbanAuditDB');
        location.reload();
    }
}
